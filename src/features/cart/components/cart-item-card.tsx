...
import { colorOptions, designs, placements } from "@/data/storefront";
...
export function CartItemCard({ item, onIncrease, onDecrease, onRemove }: CartItemCardProps) {
  ...
  const color = colorOptions.find((colorItem) => colorItem.code === item.configuredProduct.color);
      ...
      <div className="flex flex-col gap-4 sm:flex-row">
        ...
        <div className="flex flex-1 flex-col gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Badge variant="secondary">Color {color?.name ?? item.configuredProduct.color}</Badge>
